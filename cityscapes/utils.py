...
def multi_task_rg_trainer(train_loader, test_loader, multi_task_model, device, optimizer, scheduler, opt, total_epoch=200):
    ...
    def imgrad(epoch, grads, alpha=0.5, moo_alpha=0.5, rescale=2):
        g1 = grads[:,0]
        g2 = grads[:,1]
        ...
        g_0, g_m = grads.mean(1).unsqueeze(0), mgd(grads)
        ...
        def obj(x):
            ...
            return coef * (cos_max) * np.sqrt(x**2*(g11+g22-2*g12)+2*x*(g12-g22)+g22+1e-8) + (1.5 - cos_max) * 0.5*x*(g11+g22-2*g12)+(0.5+x)*(g12-g22)+g22
    ...
    for index in range(total_epoch):
        ...
        for k in range(train_batch):
            ...
            if method == "graddrop":
                ...
                g = imgrad(index, grads, alpha, moo_alpha, rescale=1)
                overwrite_grad(multi_task_model, g, grad_dims)
                optimizer.step()                
    ...
    print('Epoch: {:04d} | TRAIN: {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} || TEST: {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} | delta m: {:.4f} | TIME: {:.4f}'
            .format(index, np.mean(avg_cost[-10:, 0]), np.mean(avg_cost[-10:, 1]), np.mean(avg_cost[-10:, 2]), np.mean(avg_cost[-10:, 3]),
                    np.mean(avg_cost[-10:, 4]), np.mean(avg_cost[-10:, 5]), np.mean(avg_cost[-10:, 6]), np.mean(avg_cost[-10:, 7]), np.mean(avg_cost[-10:, 8]),
                    np.mean(avg_cost[-10:, 9]), np.mean(avg_cost[-10:, 10]), np.mean(avg_cost[-10:, 11]), np.mean(delta_m[-10:]), t1-t0))
